...
| 方法名             | 参数                                | 返回   | 说明                 |
| --------------- | --------------------------------- | ---- | ------------------ |
| add\_segments   | segments: list[str]               | None | 将文本片段向量化并添加到向量数据库中 |
| query\_similar  | query\_text: str, top\_k: int = 5 | list | 根据查询文本检索相似的前k个片段   |
| rebuild\_memory | segments: list[str]               | None | 清空原数据库，重新建立新的记忆库   |

## 输入输出规范

- 输入：文本片段（批量添加或单次查询）
- 输出：
  - `add_segments()` 无返回，仅更新数据库
  - `query_similar()` 返回最相似的片段列表
  - `rebuild_memory()` 无返回，重新初始化记忆库
